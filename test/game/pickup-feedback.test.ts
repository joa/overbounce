...
describe('the model a weapon is held as', () => {
  ...
  it('does not autoswitch when unarmed if the pickup is one Overbounce cannot fire', () => {
    ...
    const g = new Game({
      ...
      origin: [0, 0, 40],
      weapon: Weapon.NONE,
      // The lightning gun: the railgun, then the shotgun, were the examples until each was ported.
      entities: buildEntities([{ classname: 'weapon_lightning', origin: '0 0 40' }]),
      spawn: { origin: [0, 0, 40], yaw: 0 },
    });

    g.step({});
    expect(g.weapon).toBe(Weapon.NONE);
    expect(g.ps.ammo[WeaponTag.LIGHTNING]).toBe(100);
  });
  ...
  it('keeps the current weapon when the pickup is one Overbounce cannot fire', () => {
    // A deathmatch map is full of lightning guns. There is no switch to suppress
