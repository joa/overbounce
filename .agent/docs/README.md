...
| --- | --- |
| `physics-for-map-authors.md` | The measured numbers a course is built against — jump heights, gaps, rocket-jump distances, pad+rocket timing, and (section 9) Quad + battle suit rocket jumps: the 1000 impulse cap, the double rocket out of a fall (it needs 960+ of fall), and a quad rocket into a WALL as a ladder (two beat the double); how forgiving the double is by depth and exit height, both fire ticks read against the plain landing, why a lower exit exposes one-rocket lines from the rim, and that a ceiling does not stop a rocket jump (the slide keeps vz; a vertical face or playerclip-backed rock does). Section 8 also has why consecutive strafe jumps under the y lock are edge jumps: a constant view angle saturates in one hop and jumping on landing wastes the island. Section 10 (ob_circuit): plain slopes give no speed but a `common/slick` slope gives ~1000 ups, running rocket-jump reach by carried speed and drop, plain-launcher shaft climbs, the floating pad-arch selector, a strafe pad's view band (68..78, not a minimum) and why landing edges count 18 lower. Section 3 also has how a HOB behaves with no ceiling over its step (ob_grounds round 4): the step's friction bleeds any approach speed, and only a hop carrying ~600+ skips it. All from headless runs of this repo's own simulation, not from Quake lore. |
...
| `cpma-constants.md` | Every CPM constant, with the bytecode address it was read from in CPMA 1.53's shipped `.qvm`. Read this before changing any number in `cpm.ts`. Also records the one place Overbounce follows DeFRaG 1.91's promode instead: ground accel on slick/knockback. |
...
| file | what it settles |
| --- | --- |
| `side-locked-courses.md` | The renderer always showed courses from the side; the physics is full 3D. What it takes to build a course that actually earns the side camera. **Last section: rescue teleporters catch only the void** (ob_circuit round 3): never a distance gate or a plane over a walkable surface, a visible return for a real softlock, and the `teleporterPlanes` course-check that asserts it from the BSP (plus ob_grounds' two refinements: buried tops and the floor under a catch do not count). |
...
| `q3edit-mcp-traps.md` | q3edit MCP calls that fail on the first try: `label` is required, `delete` takes `targets`, a compile's `artifactPath` directory must exist, `map_compile`'s hard 180 s request timeout (a timed-out compile writes nothing) and the local q3map2 recipe that replaces it (the OpenArena pak plus images as a basepath, a stub `quark.shader`, `_q3edit_*` keys stripped), and how to take a HUD-free levelshot with `npm run shot -- --eval`. |
...
| file | what it settles |
| --- | --- |
| `playback-screens.md` | **24 numbered traps** from wiring up the playback screens, each one a bug found the expensive way. The recurring shape: anything ported from cgame assumes a clock that only rises, and playback's does not. (Its title says five; it grew.) Trap 23 is the least obvious: a default seeded from live state pins that state forever, and it looked exactly like a bug in a different file. Trap 24 is the newest: pointer capture taken on a press retargets the `click`, so a child element's `click` listener silently goes dead. |
...
| `first-bug-report.md` | The first outside bug report (2026-09-15): 5 fps in Chrome, photo mode's world "not rendered", a yaw limit request, a late jump sound. What was measured and ruled out: not the post chain (Faithful 1999 still slow), a cold shader cache costs about 1 s on nvidia with Vulkan or D3D12, photo tone toggles draw fine textured and untextured, and Quake's own mixer started new sounds at least 50 ms late (`s_mixPreStep`). What changed: the chosen adapter in the console, `body[data-gpu]` and F3, and the audio latency in the console. `powerPreference` was tried and removed: Chrome ignores it on Windows and warns about it (crbug.com/369219127). Reset view (B/Q) shipped from it; the facing indicator is being designed. Section 6, fixed: `flow`'s slick speedbelt gave 800 in DeFRaG and 400 here because DeFRaG promode keeps CPM ground accel 15 on slick (CPMA uses 1.0); proven by replaying the owner's demo tick for tick. |
| `restart-teleporters.md` | Why a teleporter that puts the player back on the spawn point ends the run in progress (owner-reported on `flow`, 2026-09-16): the criterion is the DESTINATION being within 64 units of `Game.spawn`, not the `target_init` a map may or may not put there, and it applies only while the clock is RUNNING — `flow` has one of these just past its own finish line. What DeFRaG's own timer does on that teleport is **not** verified. |
| `own-sfx.md` | The first sounds that are **not** Quake's, and the bug adding them uncovered: **the shipped overbounce test never fired on an overbounce**, twice over — the conversion lands on grounded tick 2 and the test looked only at tick 1, *and* it watched horizontal speed, which a vertical overbounce (the one `ob_basics` teaches, and the one Q3 players mean) never moves. Also why `assets/` cannot hold a shipped asset, why a demo's overbounce has to be read from the raw snapshot pair, why "never play `fight.wav`" had to go in `SoundSystem.play` rather than rest on a convention (and how capture mode tests it headlessly), and how the weighted attempt lines pick. **§5 is the other direction of the same bug**: the watch was fed `GameFrame`'s END-of-tick velocity, which a jump pad, a teleporter, knockback or a respawn may have replaced, paired with pmove's own `onGround` — so a pad landing read as an overbounce. `pmoveSpeed`/`pmoveVelocityZ` are what it reads now; `tools/diag/ob-sting.ts` shows both readings on a real `.bsp`. **§8** adds the three voice-line switches (`startsounds`/`finishsounds`/`deathsounds`), and the settings bug they uncovered **and fixed**: `settings.ts` had two writers with different semantics, and `applyHudSetting` called the live callback *instead of* persisting — so the `() => {}` stubs at the playback door silently dropped six settings (`obhelp`, `ghost`, `debugpanel`, `strafegauge`, `strafehelper`, `crosshair`), looking like a broken toggle because the panel re-rendered from storage. It now persists first and applies second, like `applyDisplaySetting` always did. §8's last section records `sensitivity`, the same shape next door and fixed with it: its slider called neither writer, so only a running course remembered a new value — it now applies on drag and persists on commit, and the trap that cost a suite run is that an undeclared `live` is a `ReferenceError` at event time, which vitest reported as a formatting error while `tsc` named it exactly. |
