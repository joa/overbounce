...
| `map` | first in the pak | Which map in the mounted paks to load, without the extension. Also selects a bare `.bsp` from `public/maps/` when no pak carries it — `ob_basics`, `ob_rockets`, `ob_crypt`, `ob_yard`, `ob_grounds`, `ob_strafes`, `ob_circuit`, `mega_rl`, `hntourney1`, `feliz-a1`. |
...
| `crosshair` | `10` | First person only. `0` hides it; otherwise one of the ten Quake III styles (`% 10`, wraparound included — `10` lands back on style `0`'s letter, the same quirk `cg_drawCrosshair 10` has). Overbounce's default is `10`, which is that wraparound: it draws the FIRST style. Quake III's own stock default is `4`, kept in the source as `DEFAULT_CROSSHAIR` beside this one. See `src/render/crosshair.ts` — the index/count math is a verified port of `CG_DrawCrosshair`; the icon art is an original recreation, since the real `.tga`s are a retail asset not in the GPL source. |
...
| `vignette` | `0` | Corner darkening, `0..1`: untouched inside 0.3 of the centre, ramping to full strength at 0.7, which is the corner, so `1` takes the corners to black and the middle of each edge halfway there. Off in play on purpose — the corners are where the next ledge is — and photo mode's LOOK panel is where it gets turned on, for the duration of the panel (the panel opens at 0; see `.agent/plans/PHOTO-MODE.md` for why it does not push its mockup's `.22`). A URL override only, not a Settings key. `0` removes the stage. |
...
| `fogdensity` | `0.6` | **Volumetric only.** Multiplier on the `depthForOpaque`-derived extinction. Well under 1, because Quake's number was authored against `RB_FogPass` — a stain applied once per surface by how far the ray travelled to reach it. Integrated properly through the volume the same coefficient is much heavier, and at `1` the volumes read as paint. Taste, not derivation; it was 0.5 until 2026-09-04. |
...
| `trail` | `modern` | The rocket's smoke trail. **`faithful`** is `CG_RocketTrail` (cg_weapons.c:325) ported: a sprite of `gfx/misc/smokepuff3.tga` on every absolute 50ms boundary, placed where the missile WAS at that instant, growing from 8 to 72 units while it fades from alpha 0.33 to nothing over two seconds, with a random fixed rotation and no drift at all — `LE_SCALE_FADE` never moves a puff, so the upward drift this used to have was an invention. A puff the view gets inside is killed, which is Quake's own overdraw guard and is visible behaviour. **`modern`** keeps that rhythm and replaces the flat sprite with a ten-step raymarch through a noise field inside each puff, so the smoke has interior structure that churns and thins with age rather than only losing opacity; it costs in proportion to the puff's area on screen and nothing at all when nothing is in flight. **`off`** is `cg_noProjectileTrail`. A missile at rest emits nothing (`TR_STATIONARY`, cg_weapons.c:352). Grenades share the rocket's trail here; `CG_GrenadeTrail` is a separate function in id's code and is not ported. The water branch (`CG_BubbleTrail`) is not either. |
