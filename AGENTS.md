...
## Editing a bundled map (`ob_basics`, `ob_rockets`, `ob_crypt`, `ob_yard`, `ob_grounds`)
...
For `ob_crypt`, `ob_yard` and `ob_grounds`, `npm run course-check [maps/<name>.bsp]` replays
...
npm run course-check              # replay ob_crypt/ob_yard/ob_grounds' obstacles against the BSP
...
npm run build-oapak               # public/<course>.pk3 for the five bundled courses
